 ...
 * These scripts are only used for compare2maps.
 *
...
function correctDate(incoming_JSDate, changedDate_Int) {
    ...
    if (period_JSON.day[0] == null && period_JSON.month[0] == null && period_JSON.year[0] == null) {
            ...
            if (period_JSON.month[0] == 0) {
                if (period_JSON.day[0] == 0) {
                    ...
                    var moduloDiff_Int = dojo.date.difference(new Date(
                        time_JSON.start[0]), incoming_JSDate, 'day') % period_JSON.day[0];
                    if (moduloDiff_Int == 0) {
                ...
                if (period_JSON.day[0] == 0) {
                    ...
                    var moduloDiff_Int = dojo.date.difference(new Date(
                        time_JSON.start[0]), incoming_JSDate, 'month') % period_JSON.month[0];
                    if (moduloDiff_Int == 0) {
                        console.log("nothing to correct");
                    } else {
                        if (moduloDiff_Int > period_JSON.month[0] / 2) { //jump to next higher 
                            incoming_JSDate = dojo.date.add(incoming_JSDate,
                                'month', period_JSON.month[0] - moduloDiff_Int);
